...
yum install -y yum-utils --quiet;
yum groups install -y 'Development Tools' 'Compatibility Libraries' --quiet;
yum install -y perl zlib-devel pcre-devel openldap-devel libxml2-devel systemd-devel openssl-devel expat-devel valgrind cmake git automake autoconf libtool lbzip2 lbzip2-utils pbzip2 bzip2-devel bzip2 libicu-devel libicu icu xz-compat-libs which python36 libpsl-devel libidn2-devel CUnit-devel python36-devel lcov.noarch --quiet;
echo 'Downloading...';
wget -qO- https://dl.bintray.com/boostorg/release/1.70.0/source/boost_1_70_0.tar.gz | tar xz;
wget -qO- https://github.com/nghttp2/nghttp2/releases/download/v1.37.0/nghttp2-1.37.0.tar.gz | tar xz;
...
./configure --quiet --prefix=/usr --disable-static --enable-lib-only --docdir=/usr/share/doc/nghttp2-1.37.0;
