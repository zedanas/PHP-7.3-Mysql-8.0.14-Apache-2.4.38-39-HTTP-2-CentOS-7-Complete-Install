...
yum -y -quiet --skip-broken install epel-release yum-utils && yum groups install 'Development Tools' 'Compatibility Libraries' \
yum -y -quiet --skip-broken install hmaccalc zlib-devel binutils-devel elfutils-libelf-devel ncurses-devel bc wget \
yum -y -quiet --skip-broken install tuned-* htop ImageMagick7 \
tuned --profile --skip-broken throughput-performance --daemon \
...
yum -y -quiet --skip-broken install https://dev.mysql.com/get/mysql80-community-release-el7-1.noarch.rpm \
yum -y -quiet --skip-broken  install mysql-server \
yum -y -quiet --skip-broken  clean all -v && yum update  \ 
