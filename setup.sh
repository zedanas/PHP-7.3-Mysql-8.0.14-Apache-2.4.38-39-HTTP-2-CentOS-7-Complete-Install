...
yum install epel-release yum-utils && yum groups install 'Development Tools' 'Compatibility Libraries' -q -v -y \
yum install hmaccalc zlib-devel binutils-devel elfutils-libelf-devel ncurses-devel bc wget -q -v -y \
yum install tuned-* htop ImageMagick7 -q -v -y \
...
cmake expat-devel libtool composer libnghttp2-devel pcre-devel sudo wget perl pcre-devel libxml2-devel -q -v -y \
yum install https://dev.mysql.com/get/mysql80-community-release-el7-1.noarch.rpm  -q -v -y \
yum install mysql-server  -q -v -y \
yum clean all -v && yum update  -q -v -y \ 
